...
class qrCode {
  public:
    ...
    static bool isValid(uint32_t version);
    static uint32_t size(uint32_t version);
    ...
    static uint32_t payloadLengthInBits(uint32_t length, uint32_t theVersion, encodingFormat theEncodingFormat);
    ...
    // static uint32_t versionNeeded(encodingFormat theEncodingFormat, uint32_t dataLength, errorCorrectionLevel theErrorCorrectionLevel);
    // static uint32_t nmbrDataModulesAvailable(uint32_t theVersion, errorCorrectionLevel theErrorCorrectionLevel);
  ...
  private:
    ...
    // static uint32_t widthHeightInModules;
    // // static bitArray<> &modules;
    // // static bitArray<> &isFunction;
    ...
    // static void drawFormatBits(uint32_t ecc, uint32_t mask);
    // static void drawVersion(uint32_t version);
