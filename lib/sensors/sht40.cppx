...
bool sht40::isPresent() {
    // 1. Check if something is connected to the I2C bus at the address of the sht40
    if (!testI2cAddress(i2cAddress)) {
        return false;
    }        //
    // 2. Check if it is a BME680 by reading the chip id register/value
    uint8_t chipidValue = readRegister(sht40::registers::chipId);
    return (sht40::chipIdValue == chipidValue);
}
...
void sht40::run() {
    if ((state == sensorDeviceState::sampling) && samplingIsReady()) {
        ...
        if (channels[barometricPressure].needsSampling()) {
            float sht40BarometricPressure = calculateBarometricPressure();
            channels[barometricPressure].addSample(sht40BarometricPressure);
...
void sht40::startSampling() {
    ...
    writeRegister(sht40::registers::ctrl_hum, 0b00000001);
    writeRegister(sht40::registers::ctrl_meas, 0b00100101);
    state = sensorDeviceState::sampling;
}

bool sht40::samplingIsReady() {
    uint8_t status    = readRegister(sht40::registers::meas_status);
...
float sht40::calculateBarometricPressure() {
