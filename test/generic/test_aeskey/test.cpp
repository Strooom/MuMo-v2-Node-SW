...
void test_initialize() {
    aesKey aKey;
    uint8_t expectedBytes[aesKey::lengthInBytes]{0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
    uint32_t expectedWords[aesKey::lengthInWords]{0x00000000, 0x00000000, 0x00000000, 0x00000000};
...
void test_setFromWordArray() {
    aesKey aKey;
    uint32_t testWords[aesKey::lengthInWords]{0x03020100, 0x07060504, 0x0B0A0908, 0x0F0E0D0C};
    ...
    const char expectedHexString[aesKey::lengthAsHexAscii + 1] = "000102030405060708090A0B0C0D0E0F";
    aKey.setFromWordArray(testWords);

    for (uint32_t index = 0; index < aesKey::lengthInWords; index++) {
        TEST_ASSERT_EQUAL_UINT32(testWords[index], aKey.getAsWord(index));
    }
    for (uint32_t index = 0; index < aesKey::lengthInBytes; index++) {
        TEST_ASSERT_EQUAL_UINT8(expectedBytes[index], aKey.getAsByte(index));
    }
